...
// src/lib/srs.js, it reads no wall clock and no storage, so the XP curve, level math, mastery
// counts, and achievement checks are deterministic and unit-testable. The useGame hook owns
// persistence (and any Date.now capture). This turns "learn the field" into a game: XP
// and levels for reviewing, mastery toward the whole glossary, per-category mastery, and an
// achievements wall, all client-side (no backend, no account). Deliberately no streaks and
// no daily quota: those are loss-aversion dark patterns (the "don't break the chain"
// coercion), the opposite of honest learning. Progress is rewarded, daily compliance is not.
