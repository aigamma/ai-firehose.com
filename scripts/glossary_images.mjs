...
// repo diagrams ship (MIT, BSD, Apache). Figures the project generates itself are also
// accepted, carried as the license token "AI-generated" with the model and date in the
// credit ("Generated by <model>, <date>"); see docs/GLOSSARY.md. Non-commercial (NC),
// no-derivatives (ND), and non-free/all-rights-reserved are rejected: the reject test runs
// first so it wins, which also means a forbidden third-party license can never slip through
// disguised as a "generated" string.
export function licenseOk(shortName) {
  ...
  return /public domain|\bpd\b|cc0|cc-?0|cc[ -]?by|creative commons|attribution|\bmit\b|\bbsd\b|apache|unlicense|\bwtfpl\b|zlib|ai[ -]?generated|generated (?:by|with|using)/.test(s);
}
...
export function licenseUrl(shortName) {
  ...
  // AI-generated figures have no canonical deed; the row sets license_url to the model's terms.
  if (/ai[ -]?generated|generated (?:by|with|using)/.test(s)) return "";
