...
The accepted explanation for polysemanticity is superposition, and the symptom-versus-cause relation is what matters. A network has strong incentive to represent more features than it has neurons, and high-dimensional geometry lets it do so by storing features as overlapping, near-orthogonal directions rather than on individual neuron axes; when several of those directions pass through one neuron, that neuron inherits a piece of each, and so appears to respond to all of them. Polysemanticity is therefore the visible symptom and superposition the underlying cause: the neuron looks confused only because it is being read in the wrong basis, mixing signals that would be clean if separated.
