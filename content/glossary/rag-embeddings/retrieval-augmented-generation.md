...
summary: A technique that grounds a language model's output in external documents by retrieving relevant passages at query time and supplying them as context, rather than relying only on the knowledge baked into its weights. Its core move is separating the knowledge from the model, facts live in an editable corpus while the model supplies the reasoning, which is what curbs hallucination, keeps answers current, and makes every claim auditable.
---

Retrieval-augmented generation, or RAG, couples a language model with a search system so the model answers from documents it is shown rather than from memory alone. A parametric model knows only what its training captured, frozen at a cutoff date and impossible to inspect or update without retraining. The keeper is what RAG does about that: it separates the knowledge from the model, so facts live in an external corpus that can be edited, versioned, and grown, while the model supplies the reasoning and fluent prose. This is why RAG has become the default architecture for question answering over private or fast-moving data.
