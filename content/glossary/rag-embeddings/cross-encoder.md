...
summary: A relevance model that takes a query and a candidate document together as a single input and outputs one relevance score, far more accurate than comparing separate embeddings but too slow to scan a whole corpus, so it is used to rerank a shortlist.
---
