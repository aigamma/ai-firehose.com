...
summary: A model that maps a piece of text (or an image, or audio) to a fixed-length vector of numbers so that semantically similar inputs land close together in the vector space, turning meaning into geometry.
---

An embedding model converts an input into a dense vector, a list of a few hundred to a few thousand real numbers, chosen so that the geometry of the vector space mirrors meaning. Two sentences that say nearly the same thing map to nearby points; two that are unrelated map far apart. This is the bridge that lets a computer compare meaning by comparing coordinates, and it is the foundation under semantic search, clustering, recommendation, and retrieval-augmented generation.
