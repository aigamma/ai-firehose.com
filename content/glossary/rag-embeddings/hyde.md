...
summary: A retrieval technique that asks a language model to draft a hypothetical answer to the query, then embeds that draft instead of the query, so the search vector sits in the same space as real documents and matches them more closely.
---
...
Instead, it first asks a language model to write a hypothetical document: a plausible answer to the query, the kind of passage that would satisfy it. That draft is then embedded, and the search runs with the draft's vector. Because the draft is written in the register and vocabulary of an actual answer, it lands near the real answers in embedding space, even though it was generated without any retrieval and may contain errors. The fabricated specifics do not matter, because the draft is used only as a search probe and is never shown to the user; its job is to point the retriever at the right neighborhood.
