...
summary: A preference-tuning method that aligns a model directly from pairs of preferred and rejected responses using a simple classification-style loss, avoiding the separate reward model and reinforcement learning of RLHF.
---

DPO, direct preference optimization, is a method for aligning a language model with human preferences that arrives at a similar goal as reinforcement learning from human feedback (RLHF) but by a much simpler route. Like RLHF, it learns from comparison data: prompts paired with a preferred response and a rejected response. Unlike RLHF, it does not train a separate reward model and does not run a reinforcement learning loop. Instead it optimizes the model directly with a single supervised-style loss, which makes it far easier to implement and more stable to train.

DPO matters because it lowered the barrier to preference alignment. The RLHF pipeline is notoriously fiddly: it requires standing up a reward model, then running Proximal Policy Optimization with careful tuning to avoid instability and reward hacking. DPO collapses that into one training stage that looks and behaves much like ordinary fine-tuning, which is why it was adopted quickly across the open-source community and many production models. It delivers competitive alignment quality with a fraction of the engineering complexity.

The central insight is mathematical: the RLHF objective, maximizing a learned reward while staying close to a reference policy, has a closed-form relationship between the optimal policy and the reward. DPO exploits this to rewrite the problem so the reward model disappears entirely. The loss simply increases the model's relative likelihood of the preferred response over the rejected one, measured against a frozen reference copy of the starting model. A temperature parameter controls how strongly preferences are enforced. The reference model anchors training so the policy improves on preferences without collapsing or drifting far from its instruction-tuned behavior.
