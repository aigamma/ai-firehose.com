...
Reasoning models made test-time compute a first-class axis of capability, which is the keeper. For years gains came from scaling parameters and training data; reasoning models show that letting a model spend more tokens thinking at inference can lift accuracy on hard problems by a large margin, a separate lever from model size, which reframes the cost of intelligence as something partly paid per query rather than only once at training.
