...
summary: A high-priority instruction placed before the conversation that sets a model's role, rules, tone, and constraints for the whole session, the developer's standing brief, given more weight than ordinary user turns through a learned instruction hierarchy. That authority is real but soft, not a hard boundary, which is why a jailbreak or prompt injection in untrusted content can still talk the model out of it.
---
