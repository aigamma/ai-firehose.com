...
The main discipline is the keeper: prompt structure. Put the stable, reusable content at the very front and the variable content (the user's actual query) at the end, so the cacheable prefix is as long as possible, since reordering a prompt so the dynamic part leads would defeat the cache entirely.
