...
It is closely related to self-consistency, with a key difference: self-consistency takes a majority vote over the candidates' final answers, while best-of-N uses an external scorer to choose, which lets it handle open-ended outputs that cannot simply be tallied. The vote needs a crisp comparable answer; the scorer needs only to rank.
