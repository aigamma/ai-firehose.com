...
summary: Constraining a model's token choices at each step so the output is guaranteed to match a formal schema or grammar, such as valid JSON or a regular expression, by masking the logits of any token that would break the structure.
---
...
The machinery sits between the model's logits and the sampler. Given a target grammar, often expressed as a JSON schema, a regular expression, or a context-free grammar, the decoder tracks the current parser state and builds a mask of allowed tokens. It applies that mask to the logits, so a token that would open a string when a number is required, or close a brace too early, simply cannot be chosen. Sampling then proceeds normally over the permitted tokens with the usual temperature and top-p controls. Because the constraint is enforced every step, the full output is guaranteed to parse.
