...
summary: Retrieval-augmented generation driven by an agent that decides when and what to retrieve, issues multiple queries, evaluates the results, and iterates, rather than a single fixed retrieve-then-generate step, so RAG can handle multi-hop and open-ended questions.
---
