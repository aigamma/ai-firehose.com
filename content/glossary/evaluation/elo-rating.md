...
You have a stream of one-on-one outcomes (this model beat that one, that one lost to a third) and you want a single number per competitor that predicts who will beat whom. That is the problem the physicist Arpad Elo solved for chess in the 1960s, and the same method now ranks language models. Each competitor holds a rating, and the difference between two ratings maps, through a logistic curve, to the probability that one beats the other: a 400-point gap, in the conventional scaling, is roughly a 10-to-1 expected win ratio. The numbers have no absolute meaning, only their differences do, which is the first thing to understand about an Elo leaderboard.
