...
It has costs and quirks, and the sharpest is worth knowing. Examples consume context-window space, which bounds how many you can include, and results can be surprisingly sensitive to which examples you pick and the order you put them in, the same task swinging in accuracy with a reshuffle. When plenty of training data exists, fine-tuning usually wins, so few-shot prompting is best seen as a fast, flexible first resort rather than the end state.
