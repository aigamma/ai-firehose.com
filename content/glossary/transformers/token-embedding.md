...
The token embedding handles meaning but not order, since the same token gets the same vector no matter where it appears. That is why a positional encoding is added to the token embedding before the first attention layer, combining what a token is with where it sits. The dimensionality of the embedding, the model dimension, is one of the architecture's basic hyperparameters and sets the width of nearly every vector that flows through the network thereafter.
