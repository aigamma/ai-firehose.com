...
This matters because outcome-only rewards have a known failure, and fixing it is the point. A model can reach the right answer through faulty or lucky reasoning and be rewarded for it, or get a hard problem almost entirely right and be punished for a single final slip; dense, step-level feedback fixes this credit-assignment problem, telling the model which specific steps helped, which is especially valuable for the long multi-step reasoning that math and code demand. PRMs are used both to guide search at inference, scoring partial solutions so a search can prune bad branches, and to train and select reasoning models.
