...
summary: A mathematical framework that bounds how much any single training example can influence a model's outputs, giving a provable privacy guarantee: an observer cannot tell whether any one individual's data was used, even with arbitrary side knowledge.
---
...
The defining property, and the reason it is trusted, is that the guarantee is mathematical and composable: the privacy cost of multiple analyses adds up in a quantifiable way, so a total budget can be tracked and enforced.
