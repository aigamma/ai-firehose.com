...
The valuable property is that the set size becomes the honest signal. On an easy, familiar input the prediction set collapses to a single confident answer; on a hard or unusual one it widens to include several, visibly flagging the model's uncertainty rather than hiding it behind a smooth probability. A wide set is the method admitting, in a principled way, that it does not know, which is exactly the behavior a downstream decision wants.
