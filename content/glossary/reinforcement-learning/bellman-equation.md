...
summary: A recursive consistency condition stating that the value of a state equals the immediate reward plus the discounted value of the states that follow, providing the mathematical foundation for nearly every reinforcement learning algorithm.
---

The Bellman equation is the central recursion of reinforcement learning and dynamic programming, named after Richard Bellman, who formalized it in the 1950s. It expresses a simple but powerful self-consistency: the long-run value of being in a state must equal the reward you collect now plus the discounted value of wherever you end up next. By turning a question about an entire future trajectory into a relationship between adjacent states, it makes long-horizon planning tractable.

This matters because the quantity an agent ultimately cares about, the total reward stretching off into the future, is impossible to evaluate directly by summing infinitely many steps. The Bellman equation breaks that intractable sum into a one-step relationship that a value-function must satisfy everywhere. Almost every reinforcement learning method is, at bottom, a different strategy for solving or approximating this equation, which is why understanding it unlocks the rest of the field.
