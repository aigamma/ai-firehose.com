...
Phase 0 (scaffold and steering harness) is in progress. The buildable React shell, the registry, the theme, and the steering docs are landing. Nothing is deployed yet and DNS is not pointed. The worker, the embedding substrate, the rotation math, and the dashboard rendering are built in Phases 1 through 6 (see the plan and the task list). When a phase lands, update this section and the relevant tier-2 doc in the same commit.
