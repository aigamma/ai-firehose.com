...
- `public/data/` The committed JSON artifacts the site reads: `constellation.json`, `clusters.json`, `spectrums.json` (positions-only; axis vectors held in the worker cache), `influence.json`, `stats.json`, plus `attention/`, `digests/`, and `glossary/` (a slim `index.json` for the list and search plus one `c/<slug>.json` hub per concept, fetched on demand). A served artifact ships only what a page renders.
