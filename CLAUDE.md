...
Phase 0 (scaffold and steering harness) is complete and committed. A Phase 4 vertical slice has landed early: the Home dashboard renders three per-kind relative-rotation boards, the constellation, the outliers strip, and the What Is New feed as lean SVG components, against a clearly-labeled synthetic seed (`worker/seed/seed.mjs`, `synthetic: true`, with a "demo data" ribbon in the UI). The build is clean (about 58 kB gzip). Still pending: the live Fly worker and source adapters (Phase 1, needs Eric's curated `sources/youtube_channels.json` and keys wired from sibling `.env` files), the embedding substrate and precompute (Phase 2), and the real rotation math (Phase 3), after which the synthetic seed is replaced by real artifacts. Nothing is deployed yet and DNS is not pointed. When a phase lands, update this section and the relevant tier-2 doc in the same commit.
