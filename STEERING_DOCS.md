...
## Glossary Onboarding (active campaign and its work queues)

The corpus-discovered trending concepts are being onboarded as durable glossary entries, an organic, scalable process that flows from the `CLAUDE.md` Current State pointer. Three committed docs run it, each describing itself and its neighbors:

- `docs/GLOSSARY_ONBOARDING.md`: the resume doc. The goal, the per-batch procedure, the voice contract, the reusable category labels, the gotchas (PowerShell quoting, the `check_doc_accuracy` count-sync), and the live status. A fresh agent resumes the campaign from here.
...
An expansion pass authors durable entries from both queues, following the `docs/GLOSSARY_ONBOARDING.md` procedure and updating its status block in the same commit.
...
| Continue or expand the glossary | `docs/GLOSSARY_ONBOARDING.md` (procedure and live status), then `docs/glossary_backlog.md` and `docs/glossary_queue.md` (what to author) |
