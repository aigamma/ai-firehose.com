...
7. **attention and rotation.** Recompute attention per entity per kind, then the relative-rotation triple (RS, ratio, momentum) and quadrant per horizon. Each board entity also carries `trail`, its trailing (ratio, momentum) trajectory (the last 8 points, passed through the same clamp and rounding as the displayed head, so the final pair equals the head), which the rotation plane draws as a comet tail. Board construction is the shared pure `computeBoards` in `worker/pipeline/boards.mjs`, so the live run (`run.mjs`) and the offline `worker/pipeline/recompute_boards.mjs` (a no-network replay of the committed store that backfills trails into the artifacts) cannot drift apart. See `docs/RAG.md`.
