...
- [ ] Trim served-artifact payloads (the "ship only what a page renders" rule). Attention boards carry rotation fields no board consumer reads; drop them in `worker/pipeline/boards.mjs`. Digests carry `movers`/`outliers` no page reads; drop them in the digest writer in `worker/pipeline/run.mjs`. Split a slim wiki-link matcher index from the heavy glossary index so `def_snippet` does not load on every route. Needs a worker run or a `worker/pipeline/slim_artifacts.mjs` migration to reshape the committed artifacts; verify byte sizes drop and every page still renders.
