...
As of **2026-06-08**: **610 durable entries**, ~**407 corpus prospects remaining** (mostly the low-attention
tail and non-durable noise; the board-visible head is now cleared). Everything below is **pushed to
`origin/main`**. After each batch run `node scripts/sync_doc_counts.mjs` to sync the public count claims
(durable, total, category) from the live artifacts so the `check_doc_accuracy` gate passes without hand-editing.
The full, explicit list of remaining prospects is committed at `docs/glossary_backlog.md`
...
Resumed 2026-06-08 (Phase B, after the full Phase A quality pass completed):
- Folded 41 RAG-reintroduced surface variants onto durable hubs (ai-agents, large-language-models, llms,
  agi, agentic-systems at 125 attention, evals, alignment, and 34 more), across two fold passes.
- Authored 10 genuinely-new board-visible concepts: model-routing, knowledge-graph, enterprise-ai,
  ai-adoption, subagents, sparse-attention, continual-learning, long-horizon-reasoning, isolated-execution,
  edge-deployment.
- Added `scripts/sync_doc_counts.mjs` to automate the count-sync gotcha.
- durable_count 600 to 610; total concepts 1058 to 1017 (folds).

Net this campaign: **59 authored entries** (49 prior plus 10 this session) and **~79 folded duplicates**.
The board-visible head is now cleared. What remains in `docs/glossary_backlog.md` (~407 prospects) is the
long tail (attention roughly 6 and below), dominated by non-durable noise (generic dev terms like api, cli,
expo, app-authentication; vague workflow phrases; specific product versions such as deepseek-v4-flash) that
the skip rule excludes, plus a thinning set of genuine low-attention concepts. Continue highest-attention
first: fold the clear synonyms (dynamic-workflows, autonomous-loops, multi-agent-scaling, local-models),
author the genuine remainder, and accept that the pure-noise residue cannot be authored and will persist in
the auto-generated list.

Phase A (the quality pass over all existing entries) is complete; see `docs/GLOSSARY_QUALITY_PASS.md`. Phase B
(this onboarding) is the active remaining work.
