...
- **Track B, prose**: the agentic daily briefing (`worker/pipeline/briefing.mjs`, `worker/pipeline/prompts/briefing.mjs`, the `digests/briefing_<horizon>.json` artifact, `src/components/Briefing.jsx`). The Watch-cycle digest (`worker/pipeline/watch_digest.mjs`, `worker/pipeline/prompts/watch_digest.mjs`, the `digests/watch.json` artifact, `src/components/WatchDigest.jsx`) is the same pattern scoped to the latest educator videos and weighted toward the recommended inner circle: an Opus synthesis cached on a hash of the video set, so a cycle with no new videos reuses the prior digest and pays nothing. Both sanitize prose through `worker/lib/text.mjs` and cite by `[n]` index into linked sources.
...
- **Track B, fully RAG-integrated**: the per-video pages (`worker/pipeline/videos.mjs`, the `public/data/videos/*` artifacts, `src/pages/VideoPage.jsx` at `/watch/:videoId`, `src/components/VideoCard.jsx`). Each video gets an on-site page with the embedded talk, an Opus 4.8 write-up richer than the classifier summary, the concepts it covers, and a Similar rail of the nearest videos by cosine similarity (one Voyage batch plus `worker/pipeline/network.mjs` (its `computeNeighbors`)). This is the embedding substrate applied to discovery. Two patterns worth copying: the committed per-video JSON is its own durable write-up store (a clone-fresh worker run reuses prior write-ups by content hash and only pays Opus for new or changed videos, backfilling in bounded batches via `VIDEO_WRITEUP_MAX`), and the stage is worker-only and keyed (NOT a keyless prebuild step, which would wipe the write-ups and neighbors on every deploy, like the briefing and the Watch digest).
