...
- **Track B, deterministic corpus-join**: the creators directory (`sources/youtube_channels.json`, the pure core `scripts/lib/directory.mjs` plus the thin IO runner `scripts/build_directory.mjs`, the `directory.json` artifact, `src/components/CreatorDirectory.jsx`). Like creators, but corpus-only with no live RSS, so the builder is a hard prebuild and generated-fresh gate, not a soft one. Two patterns worth copying: concept chips are filtered to slugs that resolve to a real glossary hub (so the integrity test passes by construction), and the `generated` stamp is derived from the data (the newest corpus publish date) rather than the wall clock, so a gated artifact is byte-identical on regeneration and never drifts by date. Onboarding is one command, `scripts/onboard_youtube.mjs` (runbook `docs/ONBOARD_YOUTUBE_CHANNEL.md`).
