...
Each hub leads with a cited, illustrative figure, chosen to teach the concept's defining insight (a geodesic shown as a great circle on a sphere, the curved analogue of a straight line), not to decorate. As of 2026-06-09 every one of the 644 durable concepts carries a figure: roughly two thirds are open-licensed images from over 70 providers, and the remaining third are AI-generated (see The Generator). The hub is a centered reading column (`.hub-page`, worldthought.com style) rather than prose pinned to the left of a wide shell.
...
**The generator (`scripts/glossary_generate_images.mjs`).** The AI-image half of the pipeline, which filled the abstract and arXiv-locked tail the open-license hunt could not. `gen` builds a house-style prompt from each gap concept's authored title and summary (a `--hint` flag injects a per-concept steer used on regeneration to fix a muddled composition or garbled labels), calls OpenAI `gpt-image-1`, and writes a small WebP (compression 82, roughly a tenth of the raw PNG) to a gitignored preview dir; `promote <accepted.json>` copies reviewed previews into `public/images/glossary/` and stages fully-attributed rows. Generation is deliberately decoupled from acceptance: every figure is viewed on the main thread before it ships, because an AI generator gets diagrams subtly wrong and a wrong teaching figure is worse than none. The same `finalize` oracle still gates: it validates real image bytes, a redistributable license (now including the `AI-generated` token), and authored `alt`.

**Variety and licensing.** Source from many open providers: Wikimedia Commons searched deeply (animations, plots, schematics, historical drawings, photos), plus scikit-learn (BSD), Our World in Data (CC BY), public-domain and government archives, and CC-licensed papers. Only redistributable licenses are accepted (public domain, CC0, CC BY, CC BY-SA); non-commercial, no-derivatives, and non-free are rejected (`licenseOk`). Figures the project generates itself are allowed too, on a plain-labeling policy: carry them as the license token `AI-generated` (which `licenseOk` accepts), name the model and date in the `credit` (`Generated by <model>, <date>`), set `provider` to the generator (for example `OpenAI`) and `license_url` to its usage terms, so the rendered attribution chip cites the image honestly as machine-made rather than passing it off as a captured real figure. The `provider` field records the spread so coverage variety is auditable. External author names are em-dash-sanitized (`cleanCredit`) so provider metadata cannot trip the writing-rule gate; authored captions and alt are scanned by the gate (`content/glossary/images.json` is in `scripts/check_no_emdash.mjs`).
